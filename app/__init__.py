...
logger.setLevel(logging.INFO)  # Asegura que Werkzeug registre solicitudes
...
# Crea la aplicación usando una fábrica
def create_app():
    ...
    # Configura las opciones de SQLAlchemy
    ...
    # Asegúrate de que el modo debug esté activado para desarrollo
    ...
    # Importa las funciones y blueprints dinámicamente dentro de create_app
    from .admin.routes import admin, update_orders_logic
    from .funcs import send_confirmation_email, fulfill_order
        ...
        with app.app_context():
            ...
            for item in items:
                if item.stock_min is not None and item.stock <= item.stock_min:
                    ...
                    nueva_alerta = Alert(message=message, category='warning', user_id=item.created_by)
    ...
    # Agrega las tareas al scheduler dentro del contexto de aplicación
    with app.app_context():
        # Añadir tareas al programador
        scheduler.add_job(
            id='update_order_status_to_free',
        ...
        # Ejecuta fix_null_master al iniciar (una vez)
        fix_null_master()
    ...
    def forgot_password():
        ...
        if form.validate_on_submit() and user:
            ...
            if user.admin:
                ...
                reset_code = form.reset_code.data.strip().lower()
                if not reset_code:
    ...
    def export_order(order_id):
        ...
        html = render_template("order_pdf.html", order=order, cliente=cliente, ci=ci, direccion_cliente=direccion_cliente,
                              emitido_por=emitido_por, direccion_negocio=direccion_negocio, nombre_negocio=nombre_negocio,
                              nit=nit, telefono=telefono, email=email)
        pdf_io = BytesIO()
    ...
    def register_admin():
        ...
        if form.validate_on_submit():
            ...
            new_user = User(name=form.name.data.strip(), email=form.email.data.lower().strip(),
                           ...
                           admin=1, email_confirmed=email_confirmed, phone=form.phone.data.strip(),
                           membership_id=int(form.membership.data), membership_expiration=membership_expiration,
                           master_key=bcrypt.generate_password_hash(master_key).decode('utf-8'))
            try:
    ...
    def register():
        ...
        if not memberships:
            ...
            return redirect(url_for('admin.configuracion'))
        form.membership.choices = [(m.id, m.name) for m in memberships]
        form.membership.data = membership.id
        ...
        if form.validate_on_submit():
            ...
            new_user = User(name=form.name.data.strip(), email=form.email.data.lower().strip(),
                           ...
                           admin=0, email_confirmed=1, created_by=current_user.id, phone=form.phone.data.strip(),
                           membership_id=int(form.membership_hidden.data), membership_expiration=current_user.membership_expiration,
                           master_key=bcrypt.generate_password_hash('TEMP_EMPLOYEE').decode('utf-8'))
            try:
                db.session.add(new_user)
                db.session.commit()
    ...
    @login_required
    def resend():
        send_confirmation_email(current_user.email)  # Ahora debería estar definido
        logout_user()
    ...
    def fulfill_order_view():
        ...
        return fulfill_order()  # Ahora debería estar definido
...
# Crea la aplicación al importar el módulo
app = create_app()
